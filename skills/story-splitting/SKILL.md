...
Use `walking-skeleton` when the chosen first slice's job is to prove a path rather than deliver a feature — greenfield work, an unproven boundary, or no build/deploy/test cycle yet. This skill picks the slice; that one builds it end to end.
...
Use `storyboard` when the work spans multiple UX surfaces or mock states; the storyboard can reveal missing screens and flow gaps that become child stories.
