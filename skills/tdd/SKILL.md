---
name: tdd
description: Test-Driven Development workflow. Use for ALL code changes - features, bug fixes, refactoring. TDD is non-negotiable. Ruby/Rails/Minitest specifics in resources/rails.md.
---
...
**Transformation Priority Premise (Uncle Bob)** — when making a test pass, pick the *highest* transformation on this list that works:

| Priority | Transformation | Example |
|----------|---------------|---------|
| 1 | {} → nil | Return nil |
| 2 | nil → constant | Return a literal value |
| 3 | constant → variable | Replace literal with a variable |
| 4 | unconditional → conditional | Add an `if` |
| 5 | scalar → collection | Use an array/map |
| 6 | statement → iteration | Add a loop/`map` |
| 7 | value → mutated value | Modify state in place |

If you find yourself jumping to transformation 5+ immediately, you likely need more RED steps to break the problem down into smaller increments.
...
- If GREEN required complex setup or extensive mocking, that's a design signal — see "Tests as Design Feedback" in the `testing` skill
...
Commit history should show clear RED → GREEN → MUTATE → KILL MUTANTS → REFACTOR → CLEANUP progression.
...
0. **Verify method visibility and callers** - before planning to test a specific method, confirm it is public AND has external callers (grep the codebase). A method declared public but with no callers outside its class is a privatization candidate. If it's private or should be, test through the public method that calls it instead.
...
- [ ] `cleanup` run over the change and looped to empty
