...
## Split Before You Plan

If the input is bigger than roughly one PR — a story, epic, feature idea, roadmap item, or vague backlog item — **run the `story-splitting` skill first**. A plan that tries to sequence work which hasn't been sliced into end-to-end user-value increments will usually drift into technical-layer steps (backend → frontend → integration) and produce PRs that don't ship value on their own.

Canonical flow: `story-splitting` → `planning` → `tdd` (→ `mutation-testing` → `refactoring`). Add `find-gaps` when the plan or AC set feels thin.

If there is no path yet — greenfield project or service, an unproven boundary, or no automated build/deploy/test cycle — run `walking-skeleton` before planning the rest. Planning features on top of an unproven architecture produces a sequence whose first real feedback arrives after every decision it should have informed.
...
3. Present the mutation testing report — and run the `test-design-reviewer` skill when the step added or changed tests. Fix surviving mutants and test anti-patterns BEFORE committing, not after. Do not skip this step.
4. **Re-read the plan's acceptance criteria** — verify each criterion is met by the implementation. If ANY criterion is not met or the implementation deviates, flag it to the user before proceeding.
5. **STOP and ask**: "Ready to commit [description]. Approve?"

Only proceed with commit after explicit approval.

### Design Decisions Require Approval

**Never make design decisions unilaterally.** If the plan specifies a design (e.g., "consolidate mappings into X"), follow it. If the plan is ambiguous or you see a better approach, present the options to the user and wait for a decision. Do not default to a pattern just because it seems cleaner — the plan is the source of truth, and the user is the decision-maker.
...
❌ **Making design decisions without approval**
- The plan is the source of truth; the user is the decision-maker
- If you see a better approach, present options and wait — do not implement your preference
...
│   ├─► VERIFY: Re-read plan ACs — flag deviations to user
